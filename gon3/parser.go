...
import (
	...
	"github.com/deonticdata/ddrdf/easylex"
)
...
func (p *Parser) expect(typ easylex.TokenType) (easylex.Token, error) {
	tok := p.next()
	if tok.Typ != typ {
		return tok, fmt.Errorf("Expected %d, got %s (type %d)", typ, tok.Val, tok.Typ)
	}
	return tok, nil
}
...
func (p *Parser) parseSubject() error {
	...
	case tokenStartCollection:
		...
		return err
	default:
		return fmt.Errorf("Expected a subject, got %v (type %d)", tok, tok.Typ)
	}
}
...
func (p *Parser) parsePredicate() error {
	...
	case tokenPNameLN, tokenPNameNS:
		...
		return err
	default:
		return fmt.Errorf("Expected predicate, got %v (type %d)", tok, tok.Typ)
	}
}
...
func (p *Parser) parseObject() error {
	...
	case tokenInteger, tokenDecimal, tokenDouble, tokenTrue, tokenFalse, tokenStringLiteralQuote, tokenStringLiteralSingleQuote, tokenStringLiteralLongQuote, tokenStringLiteralLongSingleQuote:
		...
		return err
	default:
		return fmt.Errorf("Expected object, got %v (type %d)", tok, tok.Typ)
	}
}
...
func (p *Parser) parseRDFLiteral() (*Literal, error) {
	...
	if p.peek().Typ == tokenLangTag {
		...
		case tokenPNameLN, tokenPNameNS:
			...
			lit.DatatypeIRI = iri
		default:
			return nil, fmt.Errorf("Expected an IRI or PName, got %s (type %d)", tok.Val, tok.Typ)
		}
	}
	return lit, nil
}
