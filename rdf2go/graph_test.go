...
func TestIterTriplesOrdered(t *testing.T) {
	g := NewGraph(testUri)

	g.AddTriple(NewResource("a"), NewResource("b"), NewLiteral("1"))
	g.AddTriple(NewResource("a"), NewResource("b"), NewLiteral("2"))
	g.AddTriple(NewResource("a"), NewResource("f"), NewLiteral("3"))
	// add duplicate
	g.AddTriple(NewResource("a"), NewResource("f"), NewLiteral("3"))
	...
	assert.Equal(t, "3", triples[2].Object.RawValue())
	assert.Equal(t, "2", triples[1].Object.RawValue())
	assert.Equal(t, "1", triples[0].Object.RawValue())
}
