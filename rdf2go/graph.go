...
import (
	...
	jsonld "github.com/deonticdata/ddrdf/gojsonld"
	rdf "github.com/deonticdata/ddrdf/gon3"
)
...
func (g *Graph) Len() int {
	return int(g.inserted - g.deleted)
}
...
func extractTriple(ch chan *Triple) func(key, value interface{}) bool {
...
func (g *Graph) IterTriples() (ch chan *Triple) {
	ch = make(chan *Triple)
	go func() {
		g.triples.Range(extractTriple(ch))
		close(ch)
	}()
	return ch
}
...
func (g *Graph) IterTriplesOrdered() (ch chan *Triple) {
	...
	go func() {
		...
		g.triples.Range(func(key, value interface{}) bool {
			t, ok := value.(*Triple)
			if ok {
...
func (g *Graph) insertTriple(t *Triple) {
	key := t.Hash()
	_, present := g.triples.Load(key)
	if !present {
		atomic.AddUint64(&g.inserted, 1)

		t.order = int(g.inserted)
		g.triples.Store(key, t)
	}
}
...
func (g *Graph) AddTriple(s Term, p Term, o Term) {
	g.insertTriple(NewTriple(s, p, o))
}

// Remove is used to remove a Triple object
func (g *Graph) Remove(t *Triple) {
	g.triples.Delete(t.Hash())
	atomic.AddUint64(&g.deleted, 1)
}
...
func (g *Graph) All(s Term, p Term, o Term) []*Triple {
	triples := []*Triple{}
	for triple := range g.IterTriples() {
		sMatch, pMatch, oMatch := true, true, true
		if s != nil {
			sMatch = triple.Subject.Equal(s)
		}
		if p != nil {
			pMatch = triple.Predicate.Equal(p)
		}
		if o != nil {
			oMatch = triple.Object.Equal(o)
		}
...
// GenerateJSONLD creates a interface based model of the RDF Graph.
